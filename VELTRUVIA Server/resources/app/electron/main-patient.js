...
async function tryLoadExpress(port) {
  ...
  try {
    ...
    try { const { installErrorHandlers } = await import(pathToFileURL(join(ROOT, 'src', 'errors.js')).href); installErrorHandlers({ logPath: join(sharedDataDir, 'error-log.jsonl'), name: 'veltruvia-patient' }); } catch {}
...
app.whenReady().then(async () => {
  try {
    ...
    console.log('[patient] WS upgrade forwarding ready');
